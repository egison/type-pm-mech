...
terminal auditで確認された既知の受理損失は修正済みである．整形式なsignatureの下で，
一要素tupleの要素を引数matcherへ委譲する閉じたlambdaでは，引数slotから借りたcapability variableが
局所finalization後に観察不能な$\mathsf K$へ特殊化される．以前の終端再検査は委譲先の内部を現在の
matcherによる観察と取り違えたが，修正後の9個のterminal checkは
\[
(\mathtt{true},\mathtt{true},\mathtt{true},\mathtt{true},\mathtt{true},
 \mathtt{true},\mathtt{true},\mathtt{true},\mathtt{true})
\]
であり，raw推論と公開推論の両方が成功する．matcherが観察した形を記録するshape evidenceのうち，
委譲元を表すものはprimitive patternのroot（最上位）ではないhole（部分値の処理をnext matcherへ
渡す位置）からだけ作る．
matcher自身がopaqueなconstructorを構造として直接観察する場合は
引き続き拒否する．従って全opaque capabilityを一律に許す変更ではない．

ただし，この修正は委譲holeのfinalizationで確認された損失を除くものであり，任意のraw成功がauditを通る
一般定理ではない．capability projection，evidence merge，clause capability検査を含む全traversalの
audit完全性，$\mathtt{let}$の
終端安定性，auditと独立な宣言的型付け，および拒否範囲の正確な特徴付けは公開保証に含めない．
