...
|---|---|---|---|
| 1．固有型・固有能力の保存 | ✓ 固定単相導出では固有型を後続の target 単一化から分離して保存 | ✓ matcher 式の完全な推論型を構造先行の dual check へ渡す | ✓ 全成分の完全型を hole target 単一化前に一括保存する |
| 2．各穴に完全な slot を構成 | ✓ `T-MATCHER`／Consistency (1a) の宣言規則 | ✓ match site の完全な `MatcherSlot τp τt` を構成 | ✓ 明示的な成分境界ごとに完全な `MatcherSlot κl λl` を構成 |
| 3．`Matcher μ` の双対検査 | ✓ `fresh_rename(μ) ⊑ τp` と target 単一化を分離 | ✓ `coerceMatcherToSlot` が構造先行で検査 | ✓ freeze 済み能力を既存 coercion へ送り，元の target 型を通常単一化 |
| 4．既存 `MatcherSlot` の両成分検査 | ✓ Step 3a を伝播する等式と MGU に修正済み | ✓ slot--slot の構造・target 両成分を順に単一化 | ✓ 期待 slot と両成分を検査し，得た代入を一般化前に伝播 |
| 5．型構造を保存する再帰的な変数改名 | ✓ `fresh_rename` を一文で再帰的に定義済み | ✓ matcher 側を rigid に保つ構造先行 one-way を局所的な等価実装とし，束縛領域を元 slot 変数へ固定 | ✓ 同一変数の共有を保つ再帰的改名で matcher 能力と hole 骨格を保存 |
| 6．成分検査の式構文形への非依存 | ✓ 成分境界の決定後は完全な推論型だけで検査 | ✓ matcher 位置の任意の式を推論型から coercion へ送る | ✓ 外側の明示 tuple 境界後は完全な推論型だけで検査 |
| 7．scheme lookup 後の共有変数具体化を能力保存的に制限 | ✗ P2 の中心であり未規定 | ✗ fresh lookup 後に通常引数などから能力添字を具体化できる | ✗ 通常経路と同じ scheme lookup 後の能力強化問題が残る |

1--6 は，[R12](resolved-next-matcher-slot-checking.md) の仕様に従って
Egison 本体へ反映済みである．scheme／高階フローをまたいで定義元の能力を
通常の型代入から独立にどう保持するかは 7 だけに切り分け，未解決のままとする．
