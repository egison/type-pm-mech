...
`Scheme.FreshOpening`, and the only result passed to later inference is
...
def advanceSupply (supply : InferenceBase.FreshSupply)
    (scheme : Scheme) : InferenceBase.FreshSupply :=
...
def canonicalFreshOpening (supply : InferenceBase.FreshSupply)
    (scheme : Scheme) : scheme.FreshOpening where
...
structure FreshResult (scheme : Scheme) where
...
def FreshResult.value {scheme : Scheme}
...
def freshInstantiate (supply : InferenceBase.FreshSupply)
    (scheme : Scheme) : FreshResult scheme :=
