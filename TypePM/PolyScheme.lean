...
`PolyScheme` packages the scheme-only payload syntax from `PolySyntax` with
...
def applyMeta (substitution : Subst) (scheme : PolyScheme) : PolyScheme :=
...
def instantiate (scheme : PolyScheme)
