...
**機械化が浮かび上がらせた点(その 3;論文側は未変更・要検討)**:
pp の値パターンパターン #$y は、対応する p 側 #M の M を**節選択時**に
評価する(PPP-VAL)が、M の型付け文脈は p の左側部分パターンの束縛を含み得る
(PAT-VALUE の Δ スレッディング)。左側束縛はまだ実行時環境に無いので、
#$y が第 2 引数以降の位置に立ち、かつ M が左側束縛を参照する場合、
早すぎる評価になる。標準ライブラリの全節(#$val はトップ、#$pxs は第 1 位置)は
安全側。ここでは oracle 仮定を文脈量化で受け取り、この間隙は
Thm 5.6(b) の機械化時に(該当位置の型付け文脈が実行時環境で被覆されることを
要求する形で)顕在化する。
-/
