...
It has a forgetful map to the first boundary, but is not yet generated by the W
reconstruction theorem.  The `pval` premise remains the ordinary surface
`HasTy` judgment, so these are still pattern-local boundaries; they imply
neither inference completeness nor principality.
-/
...
/-- Reconstruction evidence always satisfies the coherent terminal surface
boundary. -/
def PatternResolutionDeriv.toCoherentSurface
  ...
  | .embed rawLookup actualLookup =>
      .embed rawLookup actualLookup
  | .tuple children =>
      .tuple (PatternResolutionsDeriv.toCoherentSurface children)
  | .ctor lookup children compatible instanceTyping =>
      .ctor lookup (PatternResolutionsDeriv.toCoherentSurface children)
        compatible instanceTyping
  | .and left right =>
      .and (PatternResolutionDeriv.toCoherentSurface left)
        (PatternResolutionDeriv.toCoherentSurface right)
  | .or left right =>
      .or (PatternResolutionDeriv.toCoherentSurface left)
        (PatternResolutionDeriv.toCoherentSurface right)
  | .app lookup children instanceTyping =>
      .app lookup (PatternResolutionsDeriv.toCoherentSurface children)
        instanceTyping

/-- List reconstruction has a coherent terminal surface view pointwise. -/
def PatternResolutionsDeriv.toCoherentSurface
  ...
  | .nil => .nil
  | .cons head tail =>
      .cons (PatternResolutionDeriv.toCoherentSurface head)
        (PatternResolutionsDeriv.toCoherentSurface tail)
