...
def auditedConstants : List Name :=
  [``TypePM.Inference.annotation_freeness,
   ...
   ``TypePM.typed_never_stuck_runtime,
   ``TypePM.typed_never_stuck,
   ``TypePM.SourceTyping.never_stuck_paper1,
   ``TypePM.typed_all_timeout_iff_no_finite_eval,
   ``TypePM.SourceTyping.all_timeout_iff_no_finite_eval_paper1,
