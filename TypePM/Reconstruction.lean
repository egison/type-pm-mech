...
theorem inferExprFuel_reconstructAt
  ...
  case case5 =>
    ...
    have bodyDeriv' : ExprDeriv signature'
        ((name, Scheme.mono (terminal.prevailing.apply domain)) ::
      ...
      simpa only [Context.applySubst, List.map_cons, Scheme.applyMeta_mono]
        using bodyDeriv
    simpa [finishExpr, Subst.apply_fn] using ExprDeriv.lam bodyDeriv'
  case case9 =>
  ...
  case case27 =>
    ...
    have bodyContextEq :
      ...
      change (name, scheme.applyMeta terminal.prevailing) ::
          context'.applySubst terminal.prevailing =
        (name, signature'.generalize
          (context'.applySubst terminal.prevailing)
          (terminal.prevailing.apply valueResult.target)) ::
